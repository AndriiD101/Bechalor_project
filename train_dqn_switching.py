"""
DQN Training Script for Connect4 — Curriculum Learning
"""
...
# Define the progression of opponents. The agent will train against each
# opponent for the specified number of episodes before moving to the next.
CURRICULUM = [
    {"type": "random",     "config": {}, "episodes": 3_000},
    {"type": "rule_based", "config": {}, "episodes": 7_000},
    {"type": "alphabeta",  "config": {"max_depth": 2}, "episodes": 10_000},
    {"type": "alphabeta",  "config": {"max_depth": 4}, "episodes": 10_000},
]
...
DEFAULTS = dict(
    ...
    eps_start=1.0, 
    eps_end=0.05, 
    eps_decay=0.999_95,  # You may want to lower this slightly (e.g., 0.9999) to decay faster over 30k
    target_sync=500, 
    eval_every=1_000, 
    min_replay=5_000,
...
def train(cfg):
    ...
    # Build cumulative curriculum thresholds
    cumulative_curriculum = []
    total_eps = 0
    for phase in CURRICULUM:
        total_eps += phase["episodes"]
        cumulative_curriculum.append({
            "type": phase["type"],
            "config": phase["config"],
            "end_episode": total_eps
        })

    current_phase_idx = 0
    total_episodes = total_eps

    print(f"Starting Curriculum Training for {total_episodes} total episodes...")
    print(f"Phase 1: vs {cumulative_curriculum[0]['type']}\n")

    for episode in range(1, total_episodes + 1):
        # ── Phase Transition Logic ──
        if episode > cumulative_curriculum[current_phase_idx]["end_episode"]:
            current_phase_idx += 1
            # Reset best win rate so we save models based on the NEW opponent
            best_wr = -1.0 
            # Bump epsilon slightly to encourage exploration against the new strategy
            epsilon = max(epsilon, 0.25)
            
            phase_info = cumulative_curriculum[current_phase_idx]
            print(f"\n{'='*50}")
            print(f"🏆 CURRICULUM ADVANCE: Moving to Phase {current_phase_idx + 1}")
            print(f"⚔️ Opponent: {phase_info['type']} | Config: {phase_info['config']}")
            print(f"{'='*50}\n")

        current_phase = cumulative_curriculum[current_phase_idx]

        # ── Game Setup ──
        dqn_pid = random.choice([1, 2])
        opp_pid = 3 - dqn_pid
        
        # Instantiate opponent dynamically from factory
        opponent = create_agent(current_phase["type"], opp_pid, current_phase["config"])
        ...
        if episode % 500 == 0:
            ...
            if train_wr > best_wr:
                ...
                # Save a checkpoint specifically for this phase's peak performance
                phase_name = current_phase['type']
                torch.save(policy_net.state_dict(), save_dir / f"best_model_{phase_name}.pt")
                
                # ALSO update the single master best model
                torch.save(policy_net.state_dict(), save_dir / "best_model_cur.pt")
                marker = "  ⭐ NEW BEST (This Phase)"
                
            opp_name = current_phase["type"].upper()
            print(f"[ep {episode:>7,}] [vs {opp_name[:8]:<8}] avg_r={np.mean(ep_rewards[-500:]):+.3f}  "
                  f"eps={epsilon:.4f}  replay={len(replay):,}  train_wr={train_wr:.3f}{marker}")

        # This saves the continuous master model after EVERY episode
        torch.save(policy_net.state_dict(), save_dir / "last_model_cur.pt")

    print(f"\nTraining Complete. Final phase best win-rate: {best_wr:.3f}")
