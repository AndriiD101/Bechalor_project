...
class AlphaBetaAgent(AgentInterface):
    ...
    def _alphabeta(self, board, depth, alpha, beta, maximizing_player):
        ...
        if maximizing_player:
            ...
            for col in valid_locations:
                ...
                new_score, _ = self._alphabeta(new_board, depth - 1, alpha, beta, False)
                if new_score > value:
                    value = new_score
                    best_col = col
            ...
            for col in valid_locations:
                ...
                new_score, _ = self._alphabeta(new_board, depth - 1, alpha, beta, True)
                if new_score < value:
                    value = new_score
                    best_col = col
                beta = min(beta, value)
    ...
    def select_move(self, game):
        ...
        for col in valid_moves:
            ...
            score, _ = self._alphabeta(board_copy, self.depth - 1, float('-inf'), float('inf'), False)
            if score > best_score:
                best_score = score
                best_col = col
