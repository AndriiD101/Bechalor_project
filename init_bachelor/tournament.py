...
DQN_MODEL_PATH    = "checkpoints/best_model_rule_based.pth"   # fallback DQN path
...
def play_game(agent1, agent2) -> tuple[str, float]:
    """Play a single game using Gymnasium API.
    
    agent1 is player 1 (player in the env)
    agent2 is player 2 (opponent_agent in the env)
    """
    env = Connect4Env(opponent_agent=agent2, render_mode=None)
    obs, info = env.reset()
    t0 = time.time()
    ...
    while not (terminated or truncated):
        ...
        move = agent1.select_move(env.game)
        ...
        if info.get("illegal_move", False):
            return "Error_1", time.time() - t0
    
    winner = info.get("winner", 0)
    if winner == 0:
        return "Draw", time.time() - t0
    else:
        return f"Player_{winner}", time.time() - t0
