"""
Single-Opponent DQN Training — Connect-4
─────────────────────────────────────────
Trains DQN against a single fixed opponent (random, rule-based, or alphabeta).
Simplified version of curriculum training without phase transitions.
...
Features:
  ...
  • Epsilon decay with soft reset capability
...
DEFAULTS = dict(
    episodes=30_000,
    opponent="rule_based",  # random, rule_based, or alphabeta
    opponent_depth=2,       # for alphabeta
    batch_size=256,
...
# ─── OPPONENT FACTORY ────────────────────────────────────────────────────
def make_opponent(opponent_type: str, opponent_depth: int, opp_pid: int):
    if opponent_type == "random":
        ...
        return AlphaBetaAgent(player_id=opp_pid, max_depth=opponent_depth)
    else:
...
class MetricsTracker:
    ...
    def add(self, episode: int, train_wr: float, opponent: str, eval_stats: dict,
            epsilon: float, composite: float):
        self.history.append({
            "episode":   episode,
            "opponent":  opponent,
...
# ─── EVALUATION ─────────────────────────────────────────────────────────
def evaluate(policy_net: nn.Module, opponent_type: str, opponent_depth: int,
    ...
    for swap in (False, True):
        ...
        for _ in range(n_games // 2):
            opponent = make_opponent(opponent_type, opponent_depth, opp_pid)
            game  = Connect4Game()
            done  = False
...
def train(cfg: dict):
    ...
    print(f"Opponent: {cfg['opponent']} (depth={cfg['opponent_depth']})")
    ...
    for episode in range(episode_start + 1, cfg["episodes"] + 1):
        ...
        opponent = make_opponent(cfg["opponent"], cfg["opponent_depth"], opp_pid)
        game     = Connect4Game()
        ep_reward = 0.0
        done     = False
        ...
        if episode % cfg["eval_every"] == 0:
            ...
            eval_stats = evaluate(
                ...
                opponent_depth=cfg["opponent_depth"],
            ...
            metrics.add(episode, train_wr, cfg["opponent"], eval_stats,
                        epsilon, score)
...
# ─── ENTRY POINT ────────────────────────────────────────────────────────
def parse_args() -> dict:
    p = argparse.ArgumentParser(description="DQN for Connect-4 vs Single Opponent")
