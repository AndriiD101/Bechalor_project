"""
Self-Play DQN with Random Start States — Connect-4
───────────────────────────────────────────────────
Trains DQN against itself, but games start with random valid board states
(not empty). Gravity rules enforced: pieces must stack properly, no floating.

Features:
  • Random valid initial game states for each episode
  • Gravity enforcement: pieces stack from bottom
...
DEFAULTS = dict(
    ...
    snapshot_every=2_000,   # create new opponent snapshot every N episodes
    min_moves=5,            # minimum moves in random start state
    max_moves=20,           # maximum moves in random start state
    save_dir="checkpoints",
    resume=None,
    checkpoint_name="dqn_random_state.pt",
)
...
def generate_random_board_state(min_moves: int = 5, max_moves: int = 20):
    """
    Generate a random valid board state with pieces stacked by gravity.
    Returns (game, current_player_id) after playing random moves.
    
    Args:
        min_moves: Minimum number of random moves to play
        max_moves: Maximum number of random moves to play
    
    Returns:
        Tuple of (Connect4Game, current_player_id)
    """
    game = Connect4Game()
...
def evaluate(policy_net: nn.Module, opponent_net: nn.Module,
             n_games: int, device, eval_eps: float, cfg: dict) -> dict:
    wins = draws = losses = 0
    ...
    for swap in (False, True):
        ...
        for _ in range(n_games // 2):
            ...
            # Start from random state instead of empty board
            game, _ = generate_random_board_state(cfg["min_moves"], cfg["max_moves"])
            game.current_player = policy_pid
            done  = False
...
def train(cfg: dict):
    ...
    print(f"Random start moves: {cfg['min_moves']}-{cfg['max_moves']}")
    ...
    for episode in range(episode_start + 1, cfg["episodes"] + 1):
        ...
        # ── Game setup with random start state ──
        game, dqn_pid = generate_random_board_state(cfg["min_moves"], cfg["max_moves"])
        game.current_player = dqn_pid
        opp_pid = 3 - dqn_pid
...
# ─── ENTRY POINT ────────────────────────────────────────────────────────
def parse_args() -> dict:
    p = argparse.ArgumentParser(description="DQN Self-Play with Random Start States for Connect-4")
